...
import { Code, Eye, FileText, RefreshCw, LogOut, Terminal, Menu, X } from 'lucide-react';
...
function App() {
  ...
  const handleGenerate = async (prompt) => {
    ...
    try {
        ...
        const pollInterval = setInterval(async () => {
            await fetchFiles(); // Fetch new files
  ...
  return (
    <div className="flex h-screen bg-[#09090b] text-zinc-100 overflow-hidden font-sans selection:bg-purple-500/30">
      ...
      <div className={`
        fixed inset-y-0 left-0 z-50 w-72 bg-zinc-900/95 backdrop-blur-xl border-r border-white/5 flex flex-col transition-transform duration-300 ease-in-out md:relative md:translate-x-0 md:w-80
      ...
      `}>
        <div className="h-14 border-b border-white/5 flex items-center justify-between px-4">
            ...
            <div className="flex items-center gap-1">
                ...
                <button 
                    onClick={() => setIsSidebarOpen(false)}
                    className="md:hidden text-zinc-500 hover:text-white transition-colors p-1 hover:bg-white/5 rounded"
                >
                    <X className="w-4 h-4" />
                </button>
            </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#0c0c0e]">
        ...
        {/* Header / Tabs */}
        <div className="h-14 border-b border-white/5 flex items-center justify-between px-4 md:px-6 bg-zinc-900/30">
            <div className="flex items-center gap-3">
                ...
                <div className="flex gap-1 bg-black/20 p-1 rounded-lg border border-white/5">
                    <button 
                        ...
                        className={`flex items-center gap-2 px-3 md:px-4 py-1.5 rounded-md text-xs font-medium transition-all ${activeTab === 'preview' ? 'bg-purple-600/20 text-purple-300 shadow-sm ring-1 ring-purple-500/20' : 'text-zinc-500 hover:text-zinc-300 hover:bg-white/5'}`}
                    >
                        <Eye className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Preview</span>
                    </button>
                    <button 
                        onClick={() => setActiveTab('code')}
                        className={`flex items-center gap-2 px-3 md:px-4 py-1.5 rounded-md text-xs font-medium transition-all ${activeTab === 'code' ? 'bg-blue-600/20 text-blue-300 shadow-sm ring-1 ring-blue-500/20' : 'text-zinc-500 hover:text-zinc-300 hover:bg-white/5'}`}
                    >
                        <Code className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Code</span><span className="sm:hidden">Code</span>
                    </button>
                </div>
            </div>
            ...
            <div className="flex items-center gap-4">
                ...
                {selectedFile && (
                    <div className="hidden md:flex items-center gap-2 text-xs text-zinc-500 bg-white/5 px-3 py-1.5 rounded-full border border-white/5">
                        <FileText className="w-3.5 h-3.5" />
                        <span className="font-mono max-w-[150px] truncate">{selectedFile}</span>
                    </div>
                )}
            </div>
        </div>
        ...
        <div className="flex-1 flex flex-col relative overflow-hidden">
            ...
            <div className="absolute bottom-4 left-0 right-0 px-4 md:bottom-8 md:left-1/2 md:-translate-x-1/2 md:w-full md:max-w-3xl md:px-6 z-20">
